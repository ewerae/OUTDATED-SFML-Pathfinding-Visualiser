...
void Game::ProcessEvents() {
    ...
    while (window.pollEvent(event)) {
        ...
        if(gridManager.RunPressed() && gridManager.HasStart() && gridManager.HasObjective() && !aStar.IsInit()){
            aStar.AStarInit();
        }
...
void Game::Update(sf::Time deltaTime) {
    ...
    if(gridManager.RunPressed() && gridManager.HasStart() && gridManager.HasObjective() && !aStar.IsFinished()){
        aStar.AStarSearch();
    }
...
void Game::Render() {
    ...
    // std::string it = std::to_string(aStar.GetStart().x);
    // test.setString(it);

    // std::string it = std::to_string(aStar.GetOpenSet().size());
    // test.setString(it);

    // std::string it = std::to_string(gridManager.GetSquareStateCoordinates(SquareState::Path).size());
    if(!aStar.GetOpenSet().empty()){
        std::string it = std::to_string(aStar.GetOpenSetTop());
        test.setString(it);
    } else{
        test.setString("nope");
    }
